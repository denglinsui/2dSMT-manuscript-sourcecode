...
for mag in $(seq 1 0.5 2)
do
for (( i=1; i<=1; i++ ))
do
for (( j=0; j<=0; j++ ))
do
echo "1DUnif_h:","mag", $mag, "mu_t", ${Mu_t[$i]}, "cov_t", ${Cov_t[$j]}
...
nohup Rscript ../Simulation/Simu_1Dunif_numneigh.R --mu_type ${Mu_t[$i]} --Cov_type ${Cov_t[$j]} --magnitude $mag --est_cov False >../logerr/UnifNeigh1D$mag${Mu_t[$i]}${Cov_t[$j]}.out 2>../logerr/UnifNeigh1D$mag${Mu_t[$i]}${Cov_t[$j]}.err
