...
for (( j=0; j<=2; j++ ))
do
echo "1DUnif:","mag", $mag, "mu_t", ${Mu_t[$i]}, "cov_t", ${Cov_t[$j]}
...
nohup Rscript ../Simulation/Simu_1Dunif.R --mu_type ${Mu_t[$i]} --Cov_type ${Cov_t[$j]} --magnitude $mag --est_cov False >../logerr/Unif1D$mag${Mu_t[$i]}${Cov_t[$j]}.out 2>../logerr/Unif1D$mag${Mu_t[$i]}${Cov_t[$j]}.err
