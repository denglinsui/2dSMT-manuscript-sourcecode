...
root <- 'Result/Simulation1D_uneven_m900'
...
data_1$Method <- revalue(data_1$Method, c("2D.pis2 "="2D(ST)",
                                           #"2D.rect 4" = "2D(Rect)",
                                  "2D.sabha "="2D(SA)",
                                  "2D.ihw "="2D(IHW)",
                                  "Storey"="ST",
...
# data_1 <- data_1 %>% filter(magnitude >0.5)%>%
...
if(root%in%c('Result/Simulation1D_unif','Result/Simulation1D_unif_m2000')){
  ...
  ylim_up <- 0.95
}else if(root%in%c('Result/Simulation1D','Result/Simulation1D_m2000','Simulation1D_uneven_m900')){
  ylim_up <- 1.15
}else{
  ylim_up <- 1
}
